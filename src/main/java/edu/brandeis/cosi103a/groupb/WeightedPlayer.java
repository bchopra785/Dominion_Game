...
public class WeightedPlayer extends ParentPlayer {
    ...
    static {
        ...
        // Weight strategy: evolved optimization from medium-scale automated testing
        // Optimized values achieved 100% win rate across all configs in generation 5
        weights.put("circulation", 3.67f);      // Slightly reduced - good for cycle but not overwhelming
        weights.put("bridging", 4.82f);         // SIGNIFICANTLY INCREASED - card draw/actions are critical
        weights.put("attack", 1.59f);           // Heavily reduced - aggressive cards less valuable
        weights.put("money", 2.58f);            // Increased - resource production more important
        weights.put("defense", 1.00f);          // Reduced - minimal defense needed
        weights.put("points", 1.16f);           // Slightly increased - endgame scoring matters slightly more
